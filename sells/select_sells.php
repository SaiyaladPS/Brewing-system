...
<body>
    ...
    if (isset($_GET['del'])) {
        ...
        $btn = "UPDATE sells set o='0' where o_id='$del'";
        $query = mysqli_query($connection, $btn);
        if ($query) {
            mysqli_query($connection, "UPDATE products set qty=qty+'$oqty',amount=amount+'$amounts' where prod_id='$prod_id' ");
            echo '<script>';
            echo 'alertSuccess("ສົ່ງກັບຮຽບຮ້ອຍແລ້ວ","select_sells.php")';
            echo '</script>';
        } else {
            echo '<script>';
            echo 'alerterror("ມີບາງຢ່າງຜິດພາດ...","select_sells.php")' . $connection->error;
            echo '</script>';
        }
    }


    $count = mysqli_query($connection, " SELECT count(o_id) from sells where o='0' ");
            ...
            <div class="contianer-fulid">
                <div class="card">
                    <div class="card-header bg-light card-outline">
                        <h3 align="center">
                            <font color="green"><b> ລາຍງານສິນຄ້າຖືກຍົກເລິກ</b></font>
                        </h3>
                    </div>
                        ...
                        <div class="col-md-12">
                            ...
                            <table id="data" class="table table-bordered table-hover table-sm">
                                <?php
                                $mysql = "SELECT a.*,b.prod_id,prod_name,c.cus_id,c.fname from
                                sells as a,products as b,customer as c where a.prod_id=b.prod_id and
                                a.cus_id=c.cus_id and o='0' ";
                                    ...
                                    <tbody>
                                        ...
                                        while ($re = mysqli_fetch_array($result)) { ?>
                                            <tr>
                                                ...
                                                <!-- <td width="13%">
                                                    <a href="?del=<?php echo $re['o_id'] ?>" class="btn btn-info btndel badge">
                                                        <span class="fa fa-send"></span> ສົ່ງກັບ</a>
                                                </td> -->
                                        <?php
                                            $a++;
                                        }
